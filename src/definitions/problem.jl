...
mutable struct Problem{T,N,M,K<:Cone{T,M},D<:Space{T,N},A<:AbstractArray{T}}
	k::K
	d::D
	...
	g::A
	c::T
	function Problem(k::K, d::D, H::SparseMatrixCSC{T, Int}, P::SparseMatrixCSC{T, Int}, q::A, g::A, c::T; transpose_h=true) where {T,N,M,K<:Cone{T,M},D<:Space{T,N},A<:AbstractArray{T}}
		@assert length(q) == N
		@assert length(g) == M
		@assert size(H)[1] == (transpose_h ? M : N)
		@assert size(H)[2] == (transpose_h ? N : M)
		@assert size(P)[1] == N
		@assert size(P)[2] == N
		return new{T,N,M,K,D,A}(k,d, transpose_h ? transpose(H) : H,P,q,g,c)
	end
